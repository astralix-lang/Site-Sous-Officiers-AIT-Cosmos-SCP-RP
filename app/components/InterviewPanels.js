...
export function InterviewBookingPanel({ session, users, interviews, onAction }) {
  ...
  return <div className="interview-page">
    ...
    <div className="interview-booking-grid">
      <section className="interview-card interview-current-card">
        ...
        {current ? <>
          ...
          {booking ? <div className="interview-booked"><CheckCircle2 size={21} /><div><strong>Votre rendez-vous est confirmé</strong><span>{displaySlot(slotById.get(booking.slotId) || {})}</span></div><button type="button" className="text-danger" disabled={busy === booking.id} onClick={() => { if (window.confirm("Annuler ce rendez-vous ?")) runAction(setBusy, booking.id, onAction, { action: "cancel_interview_booking", bookingId: booking.id }); }}><XCircle size={15} /> Annuler</button></div> : <p className="interview-guidance">Choisissez l’un des créneaux ouverts par les responsables. Chaque rendez-vous dure 15 minutes.</p>}
      ...
      <section className="interview-card interview-history-card">
        ...
        <div className="interview-history-list">{completed.map((item) => <article key={item.id}><span className="interview-history-dot"><CheckCircle2 size={14} /></span><div><strong>{REASONS[item.reason]}</strong><small>Réalisé le {item.completedAt ? new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium", timeZone: "Europe/Paris" }).format(new Date(item.completedAt)) : "—"}</small>{item.completionNote && <p>{item.completionNote}</p>}</div></article>)}{!completed.length && <p className="interview-history-empty">Aucun entretien n’est encore enregistré.</p>}</div>
      </section>
    </div>
    ...
    {current?.status === "to_book" && <section className="interview-card interview-slots-card">
      ...
      {Object.keys(groupedSlots).length ? <div className="interview-slot-days">{Object.entries(groupedSlots).map(([day, daySlots]) => <article key={day}><strong>{displayDate(day)}</strong><div>{daySlots.map((slot) => <button key={slot.id} type="button" disabled={Boolean(busy)} onClick={() => runAction(setBusy, slot.id, onAction, { action: "book_interview", requirementId: current.id, slotId: slot.id })}><Clock3 size={15} /> {shortTime(slot)}</button>)}</div></article>)}</div> : <EmptyState title="Aucun créneau disponible" text="Les responsables n’ont pas encore ouvert de disponibilité. Revenez un peu plus tard." />}
    </section>}
  </div>;
}

export function InterviewManagementPanel({ users, interviews, onAction }) {
  const [busy, setBusy] = useState("");
  ...
  return <div className="interview-page interview-management-page">
    ...
    <section className="interview-card interview-dashboard-card"><div className="interview-card-head"><div><p className="eyebrow dark">TABLEAU DE SUIVI</p><h2>État des entretiens</h2><p>Les échéances mensuelles sont recalculées automatiquement après chaque entretien clôturé.</p></div><span className="interview-count">{openRequirements.length}</span></div><div className="table-wrap"><table className="interview-table"><thead><tr><th>Membre</th><th>Motif</th><th>Échéance</th><th>Rendez-vous</th><th>État</th><th aria-label="Actions" /></tr></thead><tbody>{openRequirements.map((requirement) => { const member = usersById.get(requirement.memberId); const booking = bookingByRequirement.get(requirement.id); const slot = booking ? slotsById.get(booking.slotId) : null; return <tr key={requirement.id}><td><div className="interview-member"><span className="interview-member-avatar"><UserRound size={16} /></span><div><strong>{memberName(member)}</strong><small>{member ? (member.role === "senior" ? "Sous-Officier Supérieur" : "Sous-Officier") : "Compte supprimé"}</small></div></div></td><td><strong>{REASONS[requirement.reason]}</strong></td><td><span className={`interview-due ${requirement.dueDate < parisDay() ? "late" : ""}`}>{dueText(requirement)}</span></td><td>{slot ? <span className="interview-appointment"><Clock3 size={14} />{displaySlot(slot)}</span> : <span className="interview-no-appointment">En attente du membre</span>}</td><td><RequirementPill requirement={requirement} /></td><td><div className="interview-row-actions">{booking && <button className="icon-button" type="button" title="Annuler le rendez-vous" disabled={busy === booking.id} onClick={() => { if (window.confirm("Annuler ce rendez-vous ?")) runAction(setBusy, booking.id, onAction, { action: "cancel_interview_booking", bookingId: booking.id }); }}><XCircle size={16} /></button>}<button className="secondary interview-complete" type="button" disabled={busy === requirement.id} onClick={() => complete(requirement)}><CheckCircle2 size={15} /> Clôturer</button></div></td></tr>; })}{!openRequirements.length && <tr><td colSpan="6"><EmptyState title="Aucune échéance en attente" text="Les prochains suivis apparaîtront ici automatiquement." /></td></tr>}</tbody></table></div></section>

    <section className="interview-card interview-availability-card"><div className="interview-card-head"><div><p className="eyebrow dark">CRÉNEAUX OUVERTS</p><h2>Disponibilités à venir</h2><p>Les créneaux réservés restent visibles afin de garder le planning complet.</p></div><span className="interview-duration"><Clock3 size={15} /> 15 min</span></div><div className="interview-availability-list">{upcomingSlots.map((slot) => { const booking = bookings.find((item) => item.slotId === slot.id); const member = booking ? usersById.get(booking.memberId) : null; return <article key={slot.id}><div><strong>{displaySlot(slot)}</strong><small>{booking ? `Réservé par ${memberName(member)}` : "Disponible"}</small></div>{booking ? <span className="interview-status booked"><i />Réservé</span> : <button className="icon-button danger" type="button" title="Retirer ce créneau" disabled={busy === slot.id} onClick={() => { if (window.confirm("Retirer ce créneau disponible ?")) runAction(setBusy, slot.id, onAction, { action: "delete_interview_slot", slotId: slot.id }); }}><Trash2 size={16} /></button>}</article>; })}{!upcomingSlots.length && <EmptyState title="Aucune disponibilité ouverte" text="Créez une première plage pour permettre les prises de rendez-vous." />}</div></section>
  </div>;
}
