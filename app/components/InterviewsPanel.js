  ...
  return <section className="interviews-panel" aria-label={management ? "Gestion des entretiens" : "Mes entretiens"}>
    ...
    {!data ? <div className="iv-card iv-empty" role="status">{error ? "Le suivi n’a pas pu être chargé." : "Chargement du suivi des entretiens…"}</div> : management && !data.isManager ? <p>Cette rubrique est réservée aux responsables.</p> : <>
      {management ? <>
        ...
        <section className="iv-card iv-availability-card transmission-card">
          <div className="iv-heading transmission-head"><span className="category-icon large gold"><CalendarDays size={25} /></span><div><p className="eyebrow dark">DISPONIBILITÉS</p><h2>Ouvrir mes créneaux</h2><p className="muted">Une plage de 15 h à 16 h crée quatre rendez-vous de 15 minutes à votre nom.</p></div></div>
        ...
        <section className="iv-card iv-table-card assignment-list-card">
        ...
        <section className="iv-card iv-personal transmission-card">
          <div className="iv-heading transmission-head"><span className="category-icon large gold"><CalendarDays size={25} /></span><div><p className="eyebrow dark">MON SUIVI</p><h2>{me ? REASONS[me.due.reason] : "Entretiens individuels"}</h2><p className="muted">Un temps d’échange avec un responsable pour faire le point sur votre parcours.</p></div></div>
