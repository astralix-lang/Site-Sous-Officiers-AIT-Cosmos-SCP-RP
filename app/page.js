...
const PORTAL_SECTION_REGISTRY = [
  ...
  { id: "interview_management", group: "referent", label: "Entretiens individuels", description: "Gérer les créneaux et suivre les échéances", icon: CalendarDays, access: "manager" },
  ...
  { id: "interviews", group: "global", label: "Entretiens individuels", description: "Prendre rendez-vous et consulter mon suivi", icon: CalendarDays, access: "all" },
...
function AbsenceRequestPanel({ session, users, absences, onSubmit, onExtend }) {
  ...
  const [form, setForm] = useState({ userId: session.id, startDate: today, endDate: today, reason: "" });
  ...
  async function submit(event) {
    ...
    try {
      ...
      setForm((current) => ({ ...current, startDate: nextDay, endDate: nextDay, reason: "" }));
    } catch (submissionError) {
...
function AbsenceTablePanel({ absences, users, onDelete }) {
  ...
  return (
    <section className="presence-card absence-table-card">
      ...
      <div className="table-wrap"><table className="presence-table absence-table"><thead><tr><th>Membre</th><th>Période</th><th>Raison</th><th>Statut</th><th aria-label="Actions" /></tr></thead><tbody>{orderedAbsences.map((absence) => {
...
function MeetingPanel({ session, users, meeting, history, onSave, onDelete, onSetDocument }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      attendance: members.map((user) => ({ userId: user.id, status: known.get(user.id)?.status || (user.presence === "absent" ? "absent" : "present"), note: known.get(user.id)?.note || "" })),
  ...
  async function deleteSavedMeeting(meetingId) {
    if (!window.confirm("Supprimer définitivement ce compte rendu de réunion SO ?")) return;
    setDeletingMeetingId(meetingId); setError("");
    try {
      await onDelete(meetingId);
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : "Le compte rendu n’a pas pu être supprimé.");
  ...
  async function createGoogleDocument(meetingSource = form, savedMeetingId = "") {
    ...
    try {
      ...
      const darkBackgroundApplied = await applyStyleRequests([
        { updateDocumentStyle: { documentStyle: { background: { color: { rgbColor: { red: 0.04, green: 0.055, blue: 0.047 } } } }, fields: "background" } },
      ]);
      const palette = darkBackgroundApplied
        ? { body: { red: 0.85, green: 0.88, blue: 0.83 }, title: { red: 0.93, green: 0.78, blue: 0.29 }, subtitle: { red: 0.46, green: 0.7, blue: 0.9 }, metadata: { red: 0.63, green: 0.69, blue: 0.64 }, heading: { red: 0.94, green: 0.78, blue: 0.31 }, footer: { red: 0.5, green: 0.69, blue: 0.84 } }
  ...
  return <div className="meeting-page"><form className="meeting-card" onSubmit={save}>
    ...
    <section className="meeting-section"><div className="meeting-section-title"><span className="category-icon green"><UserCheck size={20} /></span><div><h3>Présences et mots de l'effectif</h3><p>Indiquez le statut et notez les informations utiles pour chaque membre.</p></div></div><div className="meeting-attendance-table"><div className="meeting-attendance-head"><span>Membre</span><span>Présence</span><span>Mot / remarque</span></div>{members.map((user) => { const entry = form.attendance.find((item) => item.userId === user.id) || { status: "present", note: "" }; const status = entry.status; return <div className="meeting-attendance-row" key={user.id}><div className="meeting-member"><Avatar user={user} size="small" /><span><strong>{user.grade || GRADES[0]} {user.firstName} {user.lastName}</strong><small>{ROLES[user.role].label}</small></span></div><select className={`meeting-status ${MEETING_STATUS_TONES[status] || "green"}`} value={status} onChange={(event) => updateAttendance(user.id, "status", event.target.value)} title={user.presence === "absent" ? "Absence déclarée : le statut peut être ajusté pour cette réunion." : undefined}><option value="present">Présent</option><option value="absent">Absent</option><option value="late">En retard</option></select><textarea value={entry.note} onChange={(event) => updateAttendance(user.id, "note", event.target.value)} maxLength={1200} placeholder="Mot, absence, point évoqué…" /></div>; })}{!members.length && <p className="meeting-empty">Aucun Sous-Officier ou Sous-Officier Supérieur validé.</p>}</div></section>
...
function MeetingTransmissionPanel({ session, absences = [] }) {
  ...
  async function removeSavedMeeting(meetingId) {
    const state = await portalRequest("POST", { action: "delete_so_meeting_history", meetingId });
    if (Array.isArray(state?.soMeetingHistory)) setHistory(state.soMeetingHistory);
  }
...
function App() {
  ...
  function deleteAbsence(absenceId) {
    ...
    if (portalRemote) {
      portalRequest("POST", { action: "delete_absence", absenceId })
        .then((state) => { applySharedPortalState(state); flash("L’absence a été supprimée du suivi de l’équipe."); })
        .catch((error) => flash(error instanceof Error ? error.message : "La suppression n’a pas pu être réalisée."));
      return;
    }
